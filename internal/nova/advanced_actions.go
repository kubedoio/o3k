...
func (svc *Service) SuspendInstance(c *gin.Context) {
	...
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT libvirt_domain_id, status, power_state FROM instances WHERE id = $1 AND project_id::text = $2"),
		...
		go func() {
			...
			if err := svc.vmManager.SuspendVM(ctx, libvirtDomainID.String); err != nil {
				...
				if _, derr := svc.activeDB().ExecContext(dbCtx,
					database.Q("UPDATE instances SET status = $1, task_state = $2 WHERE id = $3"),
...
func (svc *Service) UnshelveInstance(c *gin.Context) {
	...
	var status string
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT status FROM instances WHERE id = $1 AND project_id::text = $2"),
		instanceID, projectID,
	).Scan(&status)
...
func (svc *Service) resizeInstance(c *gin.Context, instanceID, projectID, flavorRef string) {
	...
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT status, flavor_id FROM instances WHERE id = $1 AND project_id::text = $2"),
		instanceID, projectID,
	...
	err = svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT id FROM flavors WHERE id::text = $1 OR name = $1 LIMIT 1"),
		flavorRef,
	).Scan(&newFlavorID)
...
func (svc *Service) ConfirmResizeInstance(c *gin.Context) {
	...
	result, err := svc.activeDB().ExecContext(c.Request.Context(), database.Q(`
		...
		WHERE id = $4 AND project_id::text = $5 AND status = $6
...
func (svc *Service) MigrateInstance(c *gin.Context) {
	...
	go func() {
		...
		if _, err := svc.activeDB().ExecContext(ctx, database.Q(`
...
func (svc *Service) AddSecurityGroup(c *gin.Context) {
	...
	var sgID string
	err = svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT id FROM security_groups WHERE name = $1 AND project_id::text = $2"),
		sgName, projectID,
	).Scan(&sgID)
...
func (svc *Service) CreateBackupAction(c *gin.Context) {
	...
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT image_id FROM instances WHERE id = $1 AND project_id::text = $2"),
		instanceID, projectID,
	).Scan(&sourceImageID)
	...
	if err == nil {
		...
		if len(backupIDs) > int(rotation) {
			...
			for _, oldID := range oldBackups {
				if _, err := svc.activeDB().ExecContext(c.Request.Context(), "DELETE FROM images WHERE id = $1", oldID); err != nil {
