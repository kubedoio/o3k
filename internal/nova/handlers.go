...
type Service struct {
	...
	cache          *cache.Cache
	neutronSvc     *neutron.Service // For port allocation
	dispatcher     *tunnel.Hub
	wg             sync.WaitGroup
	ctx            context.Context
...
func NewServiceWithDB(db *sql.DB, libvirtMode string) *Service {
	ctx, cancel := context.WithCancel(context.Background())
	return &Service{
		db:          db,
		libvirtMode: libvirtMode,
...
func (svc *Service) SetDispatcher(d *tunnel.Hub) {
	svc.dispatcher = d
}
...
func (svc *Service) CreateServer(c *gin.Context) {
	...
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT id, name, vcpus, ram_mb, disk_gb FROM flavors WHERE id::text = $1 OR name = $1 LIMIT 1"),
	...
	// on this path.
	tx, err := svc.activeDB().BeginTx(c.Request.Context(), &sql.TxOptions{Isolation: sql.LevelSerializable})
	if err != nil {
	...
	queryStart = time.Now()
	if err := tx.QueryRowContext(c.Request.Context(), database.Q(`
	...
	if err := tx.QueryRowContext(c.Request.Context(), database.Q(`
		...
		FROM quotas WHERE project_id::text = $1 AND resource IN ('instances','cores','ram')
	...
	queryStart = time.Now()
	_, err = tx.ExecContext(c.Request.Context(), database.Q(`
	...
	`), instanceID, req.Server.Name, projectID, userID, flavor.ID, imageID, "BUILD", 0, keyNameValue, nowStr, nowStr)
	...
	if _, err := svc.activeDB().ExecContext(c.Request.Context(), database.Q(`
	...
	if svc.dispatcher != nil {
		...
		if err != nil {
			...
			if _, nerr := svc.activeDB().ExecContext(c.Request.Context(), "SELECT pg_notify('new_task', $1)", taskID); nerr != nil {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					...
					if _, derr := svc.activeDB().ExecContext(context.Background(),
			...
			if req.Server.KeyName != "" {
				...
				err := svc.activeDB().QueryRowContext(ctx,
					database.Q("SELECT public_key FROM keypairs WHERE user_id::text = $1 AND name = $2"),
			...
			dbCtx, dbCancel := context.WithTimeout(svc.ctx, 5*time.Second)
			defer dbCancel()
			if _, derr := svc.activeDB().ExecContext(dbCtx, database.Q(`
				UPDATE instances
...
func (svc *Service) ListServers(c *gin.Context) {
	...
	rows, queryErr := svc.activeDB().QueryContext(c.Request.Context(), database.Q(baseQuery), args...)
	if queryErr != nil {
...
// parseDBTime parses timestamps stored by SQLite (TEXT) into time.Time.
// It accepts RFC3339, SQLite's CURRENT_TIMESTAMP format, and the verbose
// string representation produced by the Go driver for time.Time values.
func parseDBTime(s string) time.Time {
	if s == "" {
		return time.Time{}
	}
	// Strip the monotonic reading suffix that the Go SQLite driver may append.
...
func (svc *Service) batchFetchAddresses(ctx context.Context, serverIDs []string) map[string]gin.H {
	...
	rows, err := svc.activeDB().QueryContext(ctx, database.Q(query), args...)
	if err != nil {
		return result
	}
	defer rows.Close()
...
func (svc *Service) ListServersDetail(c *gin.Context) {
	...
	baseQuery := `
		SELECT i.id, i.name, i.status, i.power_state, i.project_id, i.user_id,
		       ...
		       f.vcpus, f.ram_mb, f.disk_gb, f.name as flavor_name, '' as host, i.locked,
	...
	if marker := c.Query("marker"); marker != "" {
		baseQuery += fmt.Sprintf(
			database.Q(" AND (i.created_at, i.id::text) < (SELECT created_at, id::text FROM instances WHERE id = $%d)"),
			argIdx,
		)
	...
	rows, err := svc.activeDB().QueryContext(c.Request.Context(), database.Q(baseQuery), queryArgs...)
...
func (svc *Service) getInstanceAddresses(ctx context.Context, instanceID, projectID string) gin.H {
	...
	rows, err := svc.activeDB().QueryContext(ctx, database.Q(`
		...
		FROM ports p
		JOIN networks n ON p.network_id = n.id
		WHERE p.device_id = $1 AND p.project_id::text = $2
...
func (svc *Service) GetServer(c *gin.Context) {
	...
	err := svc.activeDB().QueryRowContext(c.Request.Context(), database.Q(`
		SELECT i.id, i.name, i.status, i.power_state, i.project_id, i.user_id,
		       i.flavor_id, i.image_id, i.created_at, i.updated_at, '' as host, i.launched_at,
		       i.locked, COALESCE(f.vcpus, 0), COALESCE(f.ram_mb, 0), COALESCE(f.disk_gb, 0), f.name,
		       i.task_state, i.key_name, i.fault_message
		FROM instances i
		LEFT JOIN flavors f ON i.flavor_id = f.id
		WHERE i.project_id::text = $2 AND (
	...
	metaRows, metaErr := svc.activeDB().QueryContext(c.Request.Context(),
		database.Q("SELECT meta_key, meta_value FROM instance_metadata WHERE instance_id = $1"),
		id,
	)
	if metaErr == nil {
...
func (svc *Service) DeleteServer(c *gin.Context) {
	...
	queryStart := time.Now()
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT libvirt_domain_id, user_id, status FROM instances WHERE project_id::text = $2 AND ((id::text = $1) OR (name = $1))"),
		instanceID, projectID,
	...
	if svc.neutronSvc != nil {
		ctx := c.Request.Context()
		rows, portErr := svc.activeDB().QueryContext(ctx,
			database.Q("SELECT id, mac_address, network_id FROM ports WHERE device_id = $1 AND project_id::text = $2"),
			instanceID, projectID)
		if portErr == nil {
			defer rows.Close()
			for rows.Next() {
	...
	if _, derr := svc.activeDB().ExecContext(c.Request.Context(),
	...
	_, err = svc.activeDB().ExecContext(c.Request.Context(),
		database.Q("DELETE FROM instances WHERE project_id::text = $2 AND ((id::text = $1) OR (name = $1))"),
		instanceID, projectID,
	)
...
func (svc *Service) ServerAction(c *gin.Context) {
	...
	var instanceStatus string
	err := svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT libvirt_domain_id, status FROM instances WHERE project_id::text = $2 AND ((id::text = $1) OR (name = $1))"),
		instanceID, projectID,
	...
	if svc.libvirtMode == "stub" || libvirtDomainStr == "" {
		...
		if _, ok := req["reboot"]; ok {
			...
			if _, derr := svc.activeDB().ExecContext(c.Request.Context(),
				database.Q("UPDATE instances SET status = $1, updated_at = $2 WHERE (id::text = $3 OR name = $3) AND project_id::text = $4"),
			...
			if _, derr := svc.activeDB().ExecContext(c.Request.Context(),
				database.Q("UPDATE instances SET status = $1, power_state = $2, updated_at = $3 WHERE (id::text = $4 OR name = $4) AND project_id::text = $5"),
	...
	if _, ok := req["reboot"]; ok {
		...
		if _, err := svc.activeDB().ExecContext(ctx,
			database.Q("UPDATE instances SET status = $1, power_state = $2, updated_at = $3 WHERE (id::text = $4 OR name = $4) AND project_id::text = $5"),
...
func (svc *Service) ListFlavorsDetail(c *gin.Context) {
	...
	if marker != "" {
		...
		lookupErr := svc.activeDB().QueryRowContext(ctx,
			database.Q("SELECT created_at FROM flavors WHERE id = $1"), marker).Scan(&markerTime)
		if lookupErr == nil {
...
func (svc *Service) GetFlavor(c *gin.Context) {
	...
	err := svc.activeDB().QueryRowContext(ctx,
		database.Q("SELECT id, name, vcpus, ram_mb, disk_gb, is_public FROM flavors WHERE id::text = $1 OR name = $1 LIMIT 1"),
		flavorID,
...
func (svc *Service) GetLimits(c *gin.Context) {
	...
	if err := svc.activeDB().QueryRowContext(ctx,
		database.Q(`SELECT
			COUNT(*),
	...
	quotaRows, err := svc.activeDB().QueryContext(ctx,
		database.Q(`SELECT resource, hard_limit FROM quotas WHERE project_id::text = $1`), projectID)
	if err == nil {
...
func (svc *Service) ResetServerMetadata(c *gin.Context) {
	...
	err = database.WithTx(c.Request.Context(), func(tx *sql.Tx) error {
...
func (svc *Service) RebuildInstanceAction(c *gin.Context, rebuildData interface{}) {
	...
	now := time.Now()
	_, err := svc.activeDB().ExecContext(c.Request.Context(),
		database.Q("UPDATE instances SET image_id = $1, name = COALESCE(NULLIF($2, ''), name), status = $3, updated_at = $4 WHERE id = $5 AND project_id::text = $6"),
	...
	if svc.libvirtMode == "stub" {
		...
		go func() {
			...
			defer cancel()
			if _, derr := svc.activeDB().ExecContext(ctx,
				database.Q("UPDATE instances SET status = $1, updated_at = $2 WHERE id = $3 AND project_id::text = $4"),
				"ACTIVE", time.Now(), instanceID, projectID); derr != nil {
	...
	err = svc.activeDB().QueryRowContext(c.Request.Context(),
		database.Q("SELECT id, name, flavor_id, image_id, user_id, status, created_at, updated_at FROM instances WHERE id = $1 AND project_id::text = $2"),
		instanceID, projectID,
...
func (svc *Service) LockInstanceAction(c *gin.Context) {
	...
	_, err := svc.activeDB().ExecContext(c.Request.Context(),
		database.Q("UPDATE instances SET locked = true, updated_at = $1 WHERE id = $2 AND project_id::text = $3"),
...
func (svc *Service) UnlockInstanceAction(c *gin.Context) {
	...
	// Update instance locked status
	_, err := svc.activeDB().ExecContext(c.Request.Context(),
		database.Q("UPDATE instances SET locked = false, updated_at = $1 WHERE id = $2 AND project_id::text = $3"),
		time.Now(), instanceID, projectID,
	)
	if err != nil {
...
func (svc *Service) ForceDeleteInstanceAction(c *gin.Context) {
	...
	if svc.libvirtMode == "stub" || svc.vmManager == nil {
		_, err := svc.activeDB().ExecContext(c.Request.Context(),
			database.Q("DELETE FROM instances WHERE id = $1 AND project_id::text = $2"),
			instanceID, projectID,
		)
		if err != nil {
